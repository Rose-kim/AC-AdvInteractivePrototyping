...
float n = 0, y = 0.0, s = 1.0, sunR=250;
float rad;
...
void setup() {
  ...
  fileA.loop();
  clickPlanet = loadImage("ClickthePlanet.png");
...
void mouseClicked() {
  if (overButton.check(139)) {
    sunR = rad;
  } else if (overButton.check(241)){
    meR = rad;
  }
}
