...
void serialEvent(Serial p) { 
  ...
  if (inString != null) {
    ...
    printArray(data);
    if (data.length == 4) {
      upX = map(float(data[0]), -1, 1, -1, 1);
      upY = map(float(data[1]), -1, 1, -1, 1); //convert from strint-float is okay, but to int is not that well //what if array - vector?
      zoomZ = map(int(data[3]), 0, 662, 3000, 200);
